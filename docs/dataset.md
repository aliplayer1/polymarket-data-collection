...
**Subgraph-Backed Backfill:** Historical OrderFilled fills come from Polymarket's orderbook subgraph hosted by Goldsky (with optional fallback to The Graph Network when `SUBGRAPH_API_KEY` is set). This replaces the previous Etherscan / `eth_getLogs` path — the subgraph delivers the same canonical fill records via a single GraphQL query per window, with no rate limits to manage and no daily caps to negotiate around. Subgraph rows zero out `block_number` and `log_index` (the subgraph doesn't expose them) and instead carry an `order_hash` (EIP-712, unique per fill within a transaction) that serves as the dedup discriminator. When re-running the historical phase against a window previously collected via the legacy Etherscan/RPC path, the next consolidation drops the legacy rows in favour of the new subgraph rows for the same `(market_id, timestamp_ms, token_id, tx_hash)` — WS rows are unaffected.
