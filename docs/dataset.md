...
**Integrated Execution:** Historical tick backfill is natively handled by the main `polymarket_pipeline` module exactly *before* it spins up the live WebSocket listener. There is no need to run separate backfill scripts. When the main service launches, it automatically diffs local Parquet files against the chain, fetches any missing historical ticks, saves them sequentially, and then transitions seamlessly to live mode.
